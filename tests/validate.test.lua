...
describe("mods.validate", function()
  ...
  local tests = {
    ...
    { "defined"  , 123         , nil          , "defined value expected, got no value"                  },
  ...
  describe("register", function()
    ...
    it("validate.block_device rejects non-block-device paths", function()
      assert.Function(validate.block_device)
      assert.are_same({ false, '"src" is not a valid block device path' }, { validate.block_device("src") })
    end)

    it("validate.char_device rejects non-char-device paths", function()
      assert.Function(validate.char_device)
      assert.are_same({ false, '"src" is not a valid character device path' }, { validate.char_device("src") })
    end)

    it("validate.symlink rejects non-symlink paths", function()
      assert.Function(validate.symlink)
      assert.are_same({ false, '"src" is not a valid symbolic link path' }, { validate.symlink("src") })
    end)

    it("supports optional parameter to skip nil values", function()
      assert.are_same({ true }, { validate.string(nil, nil, true) })
      assert.are_same({ false, "string expected, got no value" }, { validate.string(nil, nil, false) })
      assert.are_same({ true }, { validate(nil, "string", nil, true) })
      assert.are_same({ false, "string expected, got no value" }, { validate(nil, "string", nil, false) })
    end)
  end)
end)
